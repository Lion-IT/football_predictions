...
# Initialize logger
logger = setup_logger("etl_matches")

def run():
    while True:  # Pętla retry
        retry = False  # Flaga kontrolująca, czy proces ma być uruchomiony ponownie
        ...
        print("Mecze dostępne do analizy:")
        ...
        # Wybór meczu przez użytkownika
        try:
            selected_index = int(input("Podaj numer meczu do analizy (Enter żeby pominąć): ") or -1) - 1
        except ValueError:
            selected_index = -1

        if selected_index < 0 or selected_index >= len(available_matches):
            log_warning(logger, "Żaden mecz nie został wybrany. Domyślnie wybrany jest najnowszy dostępny mecz.")
            selected_match = available_matches[-1]  # Weź mecz z najnowszą datą
        else:
            selected_match = available_matches[selected_index]
    ...
    print("Proces analizywania zakończony.")
